...
def run_once(delay: float, timeout: int, max_bytes: int) -> dict:
    ...
    with sqlite3.connect(DB_PATH) as conn:
        query = "SELECT id, institution_id, platform, url FROM institution_sources " \
                "WHERE verification_status = 'verified' ORDER BY id"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--delay", type=float, default=2.0, help="seconds between sources; minimum 0.5")
    parser.add_argument("--timeout", type=int, default=15)
    parser.add_argument("--max-bytes", type=int, default=2_000_000)
    args = parser.parse_args()
    if not args.once:
        parser.error("use --once; schedule this command externally")
    print(json.dumps(run_once(max(0.5, args.delay), args.timeout, args.max_bytes), ensure_ascii=False))
